...
empresas_abertas_2019 <- read_csv("01_dados_tratados/empresas_abertas.csv") %>% 
                        select(-`...1`) %>% 
...
```{r}
#| warning: false
Banda_larga_municipio_2019 <- read_delim("00_bases/bandalarga_Meu_Municipio_Acessos.csv", 
...
censo_edu_superior_IES_2018 <- read_delim("00_bases/MICRODADOS_CADASTRO_IES_2018.CSV", delim = ";", escape_double = FALSE, trim_ws = TRUE, locale = locale(encoding = "latin1"))

#Exportando a base (MICRODADOS_CADASTRO_IES_2021) para tratamento: 

censo_edu_sup_IES_2018 <- 
  censo_edu_superior_IES_2018%>%
...
censo_edu_superior_cursos_2018 <-  read_excel("00_bases/MICRODADOS_CADASTRO_CURSOS_2018_reduzida.xlsx")
                                            # , delim = ";", escape_double = FALSE, trim_ws = TRUE, locale = locale(encoding = "latin1"))               
                    
censo_edu_sup_cursos_2018 <- censo_edu_superior_cursos_2018%>%
...
censo_educacao_superior_tratado_2018 <- censo_edu_sup_cursos_2018%>%
                                   left_join(censo_edu_sup_IES_2018, by = "CO_MUNICIPIO")
...
enem_2018<- read_csv("00_bases/enem_reduzida_2018.csv")
...
bolsa_familia_2019 <- read_csv("00_bases/bolsa_familia_2019.csv", 
    locale = locale(encoding = "WINDOWS-1252")) %>% 
                       filter(Referência == "01/2019")


bolsa_br_2019 <- bolsa_familia_2019 %>%
                  select(Código, `Unidade Territorial`, `Referência`, 
                           `Famílias PBF (até Out/2021)`, 
                          `Valor repassado às famílias PBF (até Out/2021)` , `Valor do Benefício médio (até Out/2021)`)%>%
                  rename(codigo_municipio_ibge = Código,
                         mes_referencia = `Referência`,
                         valor_medio_recebido_familia = `Valor do Benefício médio (até Out/2021)`,
                         familias = `Famílias PBF (até Out/2021)`,
                         valor_repassado = `Valor repassado às famílias PBF (até Out/2021)`)%>% 
...
pib_municipio_2018 <- read_excel("00_bases/pib_municipio_reduzida_2018.xlsx") 


pib_munic_2018 <- pib_municipio_2018%>%
...
pop_faixa_etaria_2018 <- read_excel("~/GitHub/lapei/05_bases_FP/01_script/00_bases/populacao_faixa_etaria_reduzida_2018.xlsx") 
                              #delim = ";", escape_double = FALSE, locale = locale(decimal_mark = ","), 
                              #trim_ws = TRUE)


pop_fx_etaria_2018 <- pop_faixa_etaria_2018%>%
                 select(ibge,perc_20_59,perc_idosa)


#população por sexo

populacao_sexo_2018 <- read_excel("00_bases/populacao_sexo_reduzida_2018.xlsx")
                                  #delim = ";", escape_double = FALSE, trim_ws = TRUE, locale = locale(decimal_mark = ",")) 


pop_sexo_2018 <- populacao_sexo_2018%>%
                   select(ibge,perc_masculino,perc_feminino,Total)%>%
                   rename(pop_total = Total)


#juntando as bases

pop_sex_fx_et_2018 <- pop_fx_etaria_2018%>%
                 left_join(pop_sexo_2018, by = "ibge") %>%
...
base_dados_nova <- 
  empresas_abertas_2019 %>% 
  left_join(homicidios_2018, by = c("cod_municipio_ibge"="cod")) %>% 
  left_join(munic_tratada, by = c("cod_municipio_ibge" = "CodMun")) %>% 
  left_join(seguranca_hidrica_out_2022, by = c("cod_municipio_ibge" = "CDMUN")) %>% 
  left_join(Banda_larga_municipio_2019, by = c("cod_municipio_ibge"="Codigo_IBGE")) %>% 
  left_join(censo_edu_sup_IES_2018, by = c("cod_municipio_ibge" = "CO_MUNICIPIO")) %>% 
  left_join(censo_edu_sup_cursos_2018, by = c("cod_municipio_ibge" = "CO_MUNICIPIO")) %>% 
  left_join(enem_2018, by = c("cod_municipio_ibge" = "CO_MUNICIPIO_ESC")) %>% 
  left_join(bolsa_br_2019, by = c("cod_municipio_ibge" = "codigo_municipio_ibge")) %>% 
  left_join(pib_munic_2018, by = c("cod_municipio_ibge" = "codigo_municipio_ibge")) %>% 
  left_join(pop_sex_fx_et_2018, by = c("cod_municipio_ibge" = "ibge")) %>% 
  relocate(SGUF, .after = cod_municipio_ibge) %>% 
  relocate(NMMUN, .after = SGUF) %>% 
  select(-nome, -CO_UF_ESC,
         -SG_UF_ESC, -`codigo_IBGE_dv`)
  

glimpse(base_dados_nova)

export(base_dados_nova, "base_dados_nova.xlsx")
