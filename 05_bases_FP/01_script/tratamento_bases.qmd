...
Os Dados foram retirados do censo da educação superior:
...
3)  Alteramos as categorias administrativas, que eram identificadas por números, para suas respectivas identificações do dicionário através da função `mutate` e `case_when`

4)  Utilizamos a função `spred` para transformar as categorias em múltiplas colunas

5)  Por intermédio da função `replace` trocamos os valores vazios `NA` por 0
...
censo_edu_sup_IES_2021 <- censo_edu_superior_IES%>%
                      ...
                      mutate(TP_CATEGORIA_ADMINISTRATIVA = case_when( 
                             ...
                             TP_CATEGORIA_ADMINISTRATIVA == 8 ~ "Privada comunitária",
                             TP_CATEGORIA_ADMINISTRATIVA == 9 ~ "Privada confessional"))%>%
                      spread(TP_CATEGORIA_ADMINISTRATIVA, n)
                      

# Trocando os valores NA por 0

censo_edu_sup_IES_2021 <- replace(x = censo_edu_sup_IES_2021, list = is.na(censo_edu_sup_IES_2021), values = 0)
...
censo_edu_sup_cursos_2021 <- censo_edu_superior_cursos%>%
                      ...
                      spread(NO_CINE_AREA_GERAL, total) 


censo_edu_sup_cursos_2021 <- replace(x = censo_edu_sup_cursos_2021, list = is.na(censo_edu_sup_cursos_2021), values = 0)
